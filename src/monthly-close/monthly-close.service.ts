...
export class MonthlyCloseService {
  ...
  async findAll(ctx: OwnershipContext, filters: any) {
    ...
    return {
      data: data.map((c: any) => ({ ...c, net_profit: Number(c.net_profit) })),
      total,
      page,
      limit,
    };
  ...
  async findOne(ctx: OwnershipContext, year: number, month: number) {
    ...
    return {
      ...close,
      income_total: Number(close.income_total),
      expense_total: Number(close.expense_total),
      cogs_total: Number(close.cogs_total),
      payroll_total: Number(close.payroll_total),
      tax_total: Number(close.tax_total),
      net_profit: Number(close.net_profit),
      cash_available: Number(close.cash_available),
    };
  }
  ...
  async preflight(ctx: OwnershipContext, year: number, month: number) {
    ...
    if (pendingExpenses > 0)
      warnings.push(`${pendingExpenses} pending expenses`);
    if (pendingPayroll > 0)
      warnings.push(`${pendingPayroll} pending payroll payments`);
    if (pendingTaxes > 0) warnings.push(`${pendingTaxes} pending tax payments`);
    if (salesWithoutCost > 0)
      warnings.push(`${salesWithoutCost} sales without cost`);

    const blocking_issues: string[] = [];
    if (month > 1 || year > new Date().getFullYear() - 1) {
      const prevOpen =
        prevClose && prevClose.status === 'CLOSED' ? false : true;
      if (prevOpen && prevClose === null) {
        // first month of history - no prior close exists, skip
      } else if (prevOpen && prevClose?.status !== 'CLOSED') {
        blocking_issues.push(
          `Previous month (${prevClose?.year ?? year}-${prevClose?.month ?? month - 1}) is not closed`,
        );
      }
    }
    if (
    ...
    if (prevClose?.status === 'CLOSED') {
      ...
      if (integrity?.status === 'DIVERGED') {
        blocking_issues.push(
          `Previous month integrity check failed: ${integrity.detail}`,
        );
      }
    }
    ...
    return {
      ...
      pending: {
        ...
        sales_without_cost: salesWithoutCost,
      },
  ...
  async close(
    ...
    await this.prisma.monthlyClose.upsert({
      ...
      create: {
        ...
        status: 'CLOSED',
        user_id: ctx.userId,
        income_total: report.net_sales,
        expense_total: report.calculated_operating_expenses,
        cogs_total: report.cogs ?? 0,
        payroll_total: report.payroll_paid,
        tax_total: report.taxes_paid,
  ...
  async reopen(
    ...
    const subsequent = await this.prisma.monthlyClose.findMany({
      where: {
        ...
        OR: [
          { year: { gt: year } },
          { year: { equals: year }, month: { gt: month } },
        ],
      },
    });
    for (const s of subsequent) {
      await this.prisma.monthlyClose.update({
        ...
        data: {
          status: 'OPEN',
          reopened_reason: `Cascade: month ${year}-${month} was reopened`,
        },
      });
    }
  ...
  async checkIntegrity(ctx: OwnershipContext, year: number, month: number) {
    ...
    if (!close || close.status !== 'CLOSED')
      return { status: 'NOT_CLOSED', year, month };

    if (
      !close.source_fingerprint ||
      close.fingerprint_version !== FINGERPRINT_VERSION
    ) {
  ...
  private async computeFingerprint(
    ...
    const expenses = await this.prisma.expense.aggregate({
      where: { ...of, date: { gte: startDate, lte: endDate } },
      _sum: { amount: true },
      _count: true,
    });
    ...
    const payroll = await this.prisma.payrollPayment.aggregate({
      where: { employee: { ...of }, paid_at: { gte: startDate, lte: endDate } },
      _sum: { net_amount: true },
      _count: true,
    });
    const taxes = await this.prisma.taxPayment.aggregate({
      where: { ...of, paid_at: { gte: startDate, lte: endDate } },
      _sum: { amount: true },
      _count: true,
    });
    const cogs = await this.prisma.costOfGoodsSold.aggregate({
      where: { income: { ...of, date: { gte: startDate, lte: endDate } } },
    ...
    return {
      expenses: {
        ...
        sum: expenses._sum.amount?.toString() ?? '0',
      },
      incomes: {
        count: incomes._count,
        sum: incomes._sum.amount?.toString() ?? '0',
      },
      payroll_payments: {
        count: payroll._count,
        sum: payroll._sum.net_amount?.toString() ?? '0',
      },
      tax_payments: {
        count: taxes._count,
        sum: taxes._sum.amount?.toString() ?? '0',
      },
      cost_of_goods_sold: {
        count: cogs._count,
        sum: cogs._sum.total_cost?.toString() ?? '0',
      },
    };
  }
}
